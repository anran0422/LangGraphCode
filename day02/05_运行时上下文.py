...
# 节点函数：处理用户消息
def node_process_message(state: AgentState, runtime: Runtime[ContextSchema]) -> dict:
...
# 节点函数：生成最终响应
def node_generate_response(state: AgentState, runtime: Runtime[ContextSchema]) -> dict:
    """生成最终响应的节点"""
...
def main():
    ...
    builder.add_node(node_process_message)
    builder.add_node(node_generate_response)

    builder.add_edge(START, "node_process_message")
    builder.add_edge("node_process_message", "node_generate_response")
