...
async def simulate_api_call(task_id: int, delay: float=1.0) -> str:
    """
        模拟API调用的异步函数
    Args:
        task_id: 任务ID
    ...
    await asyncio.sleep(delay) # 模拟网络 IO 延迟
...
async def io_task_1(state: IOState) -> dict:
    """
        IO密集型任务1 - 模拟API调用
    """
    result = await simulate_api_call(1, 1.0)
...
async def io_task_2(state: IOState) -> dict:
    """
        IO密集型任务2 - 模拟API调用
    """
    result = await simulate_api_call(2, 2.0)
    return {"results": [result]}
...
async def io_task_3(state: IOState) -> dict:
    """
        IO密集型任务3 - 模拟API调用
    """
    result = await simulate_api_call(3, 3.0)
    return {"results": [result]}
...
async def io_task_4(state: IOState) -> dict:
    """
        IO密集型任务4 - 模拟API调用
    """
    result = await simulate_api_call(4, 4.0)
    return {"results": [result]}

def node_summary(state: IOState) -> dict:
    """
        汇总节点 - 收集所有并发任务的结果
    """
    print(f"\n=== 所有任务已完成 ===")
...
async def async_version():
    ...
    tasks = [
        ...
        simulate_api_call(2, 2.0),
        simulate_api_call(3, 3.0),
        simulate_api_call(4, 4.0)
    ]
