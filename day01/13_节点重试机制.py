...
def unstable_api_call(state: State) -> Dict[str, Any]:
    ...
    attempt_count += 1
    print(f"尝试调用API，这是第 {attempt_count} 次尝试")
    # 模拟前几次尝试失败，最后一次成功
    if attempt_count < 3:
        raise Exception(f"模拟 API 调用失败（尝试了 {attempt_count} 次）")
    else:
        # 第三次会成功
        return {
            "result": f"API 调用成功，第 {attempt_count + 1} 次成功"
        }
...
def custom_retry_on(exception: Exception) -> bool:
    ...
    if "模拟 API 调用失败" in str(exception):
        print(f"捕捉到可重试异常：{exception}")
        return True

    # 对其他异常不重试
    print(f"捕捉到不可重试异常：{exception}")
    return False
...
def run_demo():
    ...
    # 添加节点
    builder1.add_node(unstable_api_call, retry_policy=RetryPolicy(max_attempts=5)) # 最多允许 5 次尝试
    builder1.add_edge(START, "unstable_api_call")
    builder1.add_edge("unstable_api_call", END)
    ...
    # 添加节点，使用自定义重试策略
    builder2.add_node(unstable_api_call, retry_policy=RetryPolicy(max_attempts=5, retry_on=custom_retry_on))
    builder2.add_edge(START, "unstable_api_call")
    builder2.add_edge("unstable_api_call", END)
    ...
    # 添加节点，使用默认重试策略
    builder3.add_node(value_error_call,retry_policy = RetryPolicy(max_attempts=3))
