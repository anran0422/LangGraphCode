...
def main():
    ...
    conn = sqlite3.connect("./sqlite_data/chat_checkpoints.sqlite",
                           check_same_thread=False) # 这里是使用锁来保证线程安全的

    sqlite_memory = SqliteSaver(conn=conn)

    graph = builder.compile(checkpointer=sqlite_memory)

    config = {
        "configurable": {
            "thread_id": "short_memory02"
        }
    }
